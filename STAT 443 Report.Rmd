...
These tools will be evaluated on their reliability and performance in indentifying an AR(1) process.

## Background
An AR(1) process, in simple terms is a process where the $t^{th}$ value depends only on the $(t-1)^{th}$ value and a i.i.d noise term. In this experiment specifically we define:
...
ar1_simulation_experiment <- function(n_sim = 1000, series_length = 100) {
  ...
  arima_results <- list(
    seasonal_true_stationary_false_stepwise_true_bic  = character(n_sim),
    seasonal_true_stationary_false_stepwise_true_aicc  = character(n_sim),
    seasonal_true_stationary_false_stepwise_true_aic  = character(n_sim),
    seasonal_true_stationary_false_stepwise_false_aic = character(n_sim),
    seasonal_false_stationary_false_stepwise_true_aic = character(n_sim),
    seasonal_true_stationary_true_stepwise_true_aic   = character(n_sim)
  )
  ...
  for (i in 1:n_sim) {
    ...
    # PP Test
    pp_pvalues[i] <- suppressWarnings(pp.test(x)$p.value)
    
    # auto.arima under multiple configurations
    arima_results$seasonal_true_stationary_false_stepwise_true_bic[i] <- 
      get_order(auto.arima(x, ic = "bic", stationary = FALSE, seasonal = TRUE, stepwise = TRUE))
    
    arima_results$seasonal_true_stationary_false_stepwise_true_aicc[i] <- 
      get_order(auto.arima(x, ic = "aicc", stationary = FALSE, seasonal = TRUE, stepwise = TRUE))
    
    arima_results$seasonal_true_stationary_false_stepwise_true_aic[i] <- 
      get_order(auto.arima(x, ic = "aic", stationary = FALSE, seasonal = TRUE, stepwise = TRUE))
    
    arima_results$seasonal_true_stationary_false_stepwise_false_aic[i] <- 
      get_order(auto.arima(x, ic = "aic", stationary = FALSE, seasonal = TRUE, stepwise = FALSE, parallel = TRUE))
    
    arima_results$seasonal_false_stationary_false_stepwise_true_aic[i] <- 
      get_order(auto.arima(x, ic = "aic", stationary = FALSE, seasonal = FALSE, stepwise = TRUE))
    
    arima_results$seasonal_true_stationary_true_stepwise_true_aic[i] <- 
      get_order(auto.arima(x, ic = "aic", stationary = TRUE, seasonal = TRUE, stepwise = TRUE))
    
  }
  
  return(list(
    pp_test_pvalues = pp_pvalues,
    arima_orders = arima_results
  ))
}

ar1_simulation_experiment(100,100)
```

\newpage

## Design
We will conduct a Monte Carlo simulation study using a known AR(1) process to evaluate the behavior of two tools:

1. **`auto.arima()`**: for identifying the correct model structure and generating accurate forecasts(80:20 train-test).
	
2. **`pp.test()`**: for testing the presence of a unit root.

Since the true data-generating process is stationary and non-seasonal (ARIMA(1,0,0)), this setup allows us to directly assess both:

- The accuracy of model order identification, and

- The quality of forecasts generated under different modeling assumptions.

The central goal is to assess how well different configurations of `auto.arima()` perform when it is allowed to consider incorrect components (e.g., seasonal terms or unnecessary differencing), as well as to access `pp.test()`'s ability to detect unit roots when the true root is close to 1. This reflects real-world use cases where users may not be able to confidently assume stationarity or non-seasonality.

Thus, for all of out experiments we will set `stationary = FALSE` and `seasonal = TRUE` which does not force auto.arima() to compensate for it, but just includes it in its search space to investigate the effects of:

- Information criteria: AIC, AICc, and BIC 

- Search strategies: stepwise vs exhaustive

- Series length: 50, 100, and 250

Together, these tests will allow us to examine how resilient each configuration is to loose assumptions and how they behave under different sample sizes.
